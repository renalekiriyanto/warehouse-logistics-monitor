...
async function fetchBacklogsApi() {
  ...
  try {
    ...
    const mappedItems = rawItems.map((item: any) => ({
      ...
      days: Number(item.days) || Number(item.hari_penumpukan) || 1,
      reason: item.reason || item.alasan || item.alasan_backlog || 'Alamat tidak ditemukan',
      courier: item.courier || item.kurir || 'Andi',
      status: item.status || 'delayed',
      date: item.date || '2026-05-30'
    }));

    store.importData('backlog', mappedItems);
    apiSuccess.value = true;
    lastFetchTime.value = new Date().toLocaleTimeString('id-ID');
  } catch (err: any) {
    console.error('Backlog API Fetch failed:', err);
...
// Reminder to Courier modal popup states for backlog
...
const courierStats = computed(() => {
  ...
  // Calculate backlog packages matching the driver
  const backlogPackages = store.backlog.filter(x => x.courier === name);
  const totalExpediteDelivering = backlogPackages.length;
  // Fallback calculations for backlog status info
  const delivered = 0; // Backlog items are inherently pending/delayed/undelivered
  const onhold = totalExpediteDelivering;
  ...
  const doneDelivering = `0 dari ${totalExpediteDelivering} Paket Selesai (Backlog)`;
...
function sendReminderNotification() {
  ...
  setTimeout(() => {
    ...
    showNotification(`Pengingat backlog berhasil dikirim kepada kurir ${activeRemindedCourier.value}!`, 'success');
  }, 1000);
}
...
// Columns definition for backlog: no resi, driver name/latest operator, status, action
const columns = [
...
<template>
  <div id="backlog-menu-comp" class="space-y-6">
    ...
    <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-xs space-y-4">
        ...
        <div class="flex items-start gap-3">
          ...
          <div>
            <div class="flex items-center gap-2">
              <h3 class="text-sm font-bold text-slate-900 border-none">Integrasi FMS Backlog (REST API)</h3>
              <span 
            ...
            <p class="text-xs text-slate-500 max-w-lg leading-relaxed mt-1">
              Data backlog dapat disinkronkan secara real-time dari central server atau diunggah manual via CSV:
              <code class="bg-slate-100 px-1 py-0.5 rounded text-xs font-mono text-blue-600">GET /api/backlogs</code>.
            </p>
          </div>
        </div>
      ...
      <div v-if="apiSuccess" class="p-3.5 bg-green-50 border border-green-200 rounded-lg flex items-start gap-2.5">
        ...
        <div class="text-xs text-green-800 space-y-0.5 leading-relaxed">
          ...
          <p>Membaca data backlog logistik aktif pada pukul <b>{{ lastFetchTime }}</b>.</p>
        </div>
      </div>
      ...
      <div v-if="apiError" class="p-3.5 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-2.5">
        ...
        <div class="text-xs text-amber-900 space-y-1 leading-relaxed">
          <p class="font-bold">Gagal Terhubung ke http://127.0.0.1:8000/api/backlogs</p>
          <p class="text-2xs text-slate-500">Pastikan REST API lokal Anda berjalan di port 8000. Sistem menggunakan data lokal browser agar Anda tetap dapat melakukan operasi pengiriman harian secara lancar.</p>
        </div>
      </div>
    </div>
    ...
    <FileUpload 
      menu-key="backlog"
      menu-label="Import Data Backlog (CSV/Excel)"
    ...
    <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-xs space-y-4">
      <div class="flex items-center gap-2">
        ...
        <h4 class="text-xs font-bold text-slate-800 uppercase tracking-wider">Filter Range Tanggal Backlog</h4>
      </div>
    ...
    <div 
      ...
      id="backlog-reminder-backdrop"
      class="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 transition-all"
    >
      <div id="backlog-reminder-modal" class="bg-white border border-slate-250 rounded-2xl max-w-sm w-full shadow-2xl overflow-hidden">
        <!-- Modal Header -->
        <div class="px-5 py-4.5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div class="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p class="text-[9px] text-slate-400 font-bold uppercase tracking-wide">FMS Dispatcher Info (Backlog)</p>
            </div>
          </div>
          <button 
            type="button" 
        ...
        <div v-if="courierStats" class="p-5 space-y-4 text-xs">
          ...
          <div class="grid grid-cols-2 gap-3.5 font-sans">
            <div class="border border-slate-200 rounded-xl p-3 bg-slate-50/50">
              <span class="text-[9px] font-bold text-slate-400 uppercase tracking-wide block">Backlog Active</span>
